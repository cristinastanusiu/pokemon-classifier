...
def GetHOG(imgrgb):
    ...
    winSigma = 4.
    histogramNormType = 0
    L2HysThreshold = 2.0000000000000001e-05
    gammaCorrection = 0
    nlevels = 64
    ...
    padding = (8,8)
    locations = ((10,20),)
    hist = hog.compute(imgrgb,winStride,padding,locations) 
    return hist

def Harrispoint(gray):
    dst = cv2.cornerHarris(gray,2,3,0.04)
    #result is dilated for marking the corners, not important
    dst = cv2.dilate(dst,None)

    thres = dst>0.08*dst.max()
    harris_vector = np.array([[0]])
    
    for i in range(dst.shape[0]):
        for j in range(dst.shape[1]):
            if thres[i,j]:
                harris_vector = np.concatenate((harris_vector,np.array([[dst[i,j]]])))
                if harris_vector.shape[0] > no_features:
                    break
        if harris_vector.shape[0] > no_features:
            break
...
def main():
    ...
    imgrgb = cv2.imread('./pokemon/Pikachu/18.jpg')
    gray= cv2.cvtColor(imgrgb,cv2.COLOR_BGR2GRAY)
    # gray = np.float32(gray)
    

    # print Harrispoint(gray).shape    
    ...
    for train_index, test_index in kf:
        ...
        X_train_pca = X_train#pca.transform(X_train)
        ...
        for y_test_img in y_test_img_class:
            for resized in pyramid(img, scale=1.5):
                ...
                for (x, y, window) in sliding_window(resized, stepSize=32, windowSize=(winW, winH)):
                    ...
                    test_img = Harrispoint(crop_img);
                    # temp = np.asarray(crop_img, dtype=np.uint32)
                    test_img = test_img.reshape(1, -1)

                    if test_img.shape[1] != X_train_pca.shape[1]:
                        break

                    test_img_pca = test_img#pca.transform(test_img)
